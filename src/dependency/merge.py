...
def merge_tag(inf_dir, outf_dir):
    ...
    for id, data in tqdm(enumerate(datas)):
        ...
        for texts in texts_list:
            ...
            # DP_tags: [{구문구조: 'R', 'VNP', 'L', 'VP', 'S', 'AP', 'NP', 'DP', 'IP', 'X'}, {기능구조: 'None', 'MOD', 'CNJ', 'AJT', 'OBJ', 'SBJ', 'CMP'}]
            ...
            # 청크 표현 기준에 따라 변경
            #tag_list = [MOD+ AJT+ CMP] # 기능 구조
            tag_list = [x for x in [np_list, dp_list, vp_list, ap_list, ip_list, r_list, l_list, s_list, x_list] if len(x) != 0] # 구문 구조
            #tag_list = [MOD+ AJT+ CMP]+[x for x in [np_list, dp_list, vp_list, ap_list, ip_list, r_list, l_list, s_list, x_list] if len(x) != 0]
...
if __name__ =='__main__':
    ...
    inf_dirs = ["./../../data/koala/koala_klue_nli_dev.json", "./../../data/koala/koala_klue_nli_train.json"]
    outf_dirs = ["./../../data/merge/merge_klue_nli_dev.json", "./../../data/merge/merge_klue_nli_train.json"]
